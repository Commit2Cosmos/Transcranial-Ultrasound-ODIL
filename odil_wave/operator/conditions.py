"""Boundary conditions and sponge damping for finite-difference stencils."""
...
class Sponge(Conditions):
    ...
    ``σ_sum D_t[u] + σ_prod u``, with discrete FD symbol ``λ_t``::

        fu + w * (σ_sum_nd * λ_t * u + σ_prod_nd * u)

    ``λ_t`` is broadcast ``(1, nf, 1, 1)`` from :class:`FrequencySelection`.
    """
...
# Back-compat alias (docs should prefer Sponge)
PML = Sponge