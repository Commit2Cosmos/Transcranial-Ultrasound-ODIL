...
class VelocityModel:
    ...
    def __init__(self, grid: Grid, profile: str = "homogeneous",
                 base: float = 1.0, contrast: float = 0.4, **profile_kwargs):
        self.grid = grid
    ...
    def _build(self) -> torch.Tensor:
        ...
        if self.profile == "overdensity":
            ...
            return torch.where(mask,
                               torch.full_like(base_field, self.base + self.contrast),
                               base_field)
    ...
    @property
    def c_max(self) -> float: return float(self.c.max())

    @property
    def c_min(self) -> float: return float(self.c.min())

    def show(self, ax=None, title: Optional[str] = None,
             vmin: Optional[float] = None, vmax: Optional[float] = None,
             show_pml: bool = True):
        if ax is None:
            _, ax = plt.subplots(figsize=(5.5, 4.5))
        (xmin, xmax), (ymin, ymax) = self.grid.extent
        im = ax.imshow(self.c.cpu().numpy().T, origin="lower",
