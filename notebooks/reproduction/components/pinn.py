...
def get_device():
    return torch.device('cuda' if torch.cuda.is_available() else 'cpu') # set the device
...
def collocation_points(n_interior=8192, n_boundary=768):
    n_each = n_boundary // 4
...
def training_plot(model: nn.Module, xx: torch.Tensor, tt: torch.Tensor, step: int) -> None:
    ...
    im0 = axs[0].imshow(
        ...
        cmap='RdBu_r', vmin=-0.5, vmax=0.5, aspect=4
    )
    im1 = axs[1].imshow(
        error,
        extent=[xx.min().item(), xx.max().item(), tt.max().item(), tt.min().item()],
        cmap='RdBu_r', aspect=4
    )
...
def plot_losses(losses: dict):
    if len(losses['L']) == 0:
        raise BufferError("Detected history of length 0. Please train before trying to plot.")
    fig, axs = plt.subplots(2, 3, figsize=(12, 8))

    titles = [r"$L(u_i^n)$", r"$\|\mathcal{R}_\mathrm{PDE}\|^2$", r"$\|\mathcal{R}_\mathrm{IC}\|$", 
              r"$\|\mathcal{R}_\mathrm{left}\|$", r"$\|\mathcal{R}_\mathrm{right}\|$", r"$\|\mathcal{R}_\mathrm{vel}\|$"]
...
def train(config: PINNConfig):
    ...
    t_physics, x_physics, t_ic, x_ic, t_left, x_left, t_right, x_right, t_vel, x_vel = collocation_points()

    t_test = torch.linspace(t_0, t_N, 100).to(device)
    x_test = torch.linspace(x_0, x_I, 100).to(device)

    lbfgs = torch.optim.LBFGS(pinn.parameters(), lr=config.lr, line_search_fn="strong_wolfe")
    ...
    for i in range(config.epochs):
        ...
        print(f"Training duration: {time.time() - start:.1f}s")
...
if __name__ == "__main__":
    ...
    model = PINN()
    summary(model.net, input_size=(1, 2), col_names=["input_size", "output_size", "num_params"])
