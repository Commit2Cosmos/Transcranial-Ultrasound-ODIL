...
def get_exact(tt: np.ndarray, x: np.ndarray) -> np.ndarray:
    ...
    for t_idx, t in enumerate(tt):
        ...
        for i in ii:
            ...
            u[t_idx] += np.cos((x + t - 0.5) * k)  # rightward NOTE: they specify + 0.5 but plot - 0.5 for this part
        
    u /= 2 * len(ii)  # 1/10 prefactor from the paper
    return u

def plot_1d(uu: np.ndarray, xx: np.ndarray, tt:np.ndarray, title: str="Exact solution") -> None:
