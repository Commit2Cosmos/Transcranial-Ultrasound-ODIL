...
class DiscretePDELoss:
    ...
    def _compute_residuals(self, p_flat: torch.Tensor) -> torch.Tensor:
        ...
        for k in range(1, 6):
            ...
            g_vel += 0.1 * kpi * (torch.sin((xs + 0.5) * kpi) - torch.sin((xs - 0.5) * kpi))  # u_t(x,0) = 0
            g_left += 0.1 * (torch.cos((self.u.x_0 - ts + 0.5) * kpi) + torch.cos((self.u.x_0 + ts - 0.5) * kpi))
            g_right += 0.1 * (torch.cos((self.u.x_I - ts + 0.5) * kpi) + torch.cos((self.u.x_I + ts - 0.5) * kpi))
    ...
    def _log(self, r: torch.Tensor) -> None:
        ...
        idx = 0
        self.history['R_pde'].append(r[idx:idx+n_pde].norm().item());   idx += n_pde
        self.history['R_ic'].append(r[idx:idx+n_ic].norm().item());     idx += n_ic
        self.history['R_vel'].append(r[idx:idx+n_vel].norm().item());   idx += n_vel
        self.history['R_left'].append(r[idx:idx+n_left].norm().item()); idx += n_left
...
def run_odil_newton(u, functional):
    ...
    for step in range(10):
        ...
        functional._log(r)
        delta = torch.linalg.lstsq(J, -r).solution # solve least squares problem, equivalent to newton
        x = x + delta # update
...
def run_odil(init_values, method="L-BFGS-B", field_plot_title="ODIL", history_plot_title="Residual history", maxiter=50000, ftol=1e-12, gtol=1e-8) -> Wavefield:
    # setup
...
if __name__ == "__main__":
    ...
    run_odil(init_values='zeros', method="Newton", field_plot_title="ODIL (Newton's method)", history_plot_title="Residual history, Newton")